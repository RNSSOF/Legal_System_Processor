...
الميزات الرئيسية
التقسيم الدفعي (Batch Splitting): تقسيم آلي للملفات إلى وحدات ذرية قانونية (ALUs) لكل مادة.
التنظيم الهيكلي: إنشاء مجلدات فرعية لكل وثيقة لضمان النظام والسهولة في التعامل مع الآلاف من الوثائق.
الإثراء الذكي (LLM Enrichment): إضافة بيانات وصفية غنية (ملخصات، كلمات مفتاحية، تصنيفات).
الترابط الآلي: ربط المواد ببعضها البعض (المادة السابقة والتالية) لتسهيل التنقل السياقي.
الإعداد والتشغيل
المتطلبات الأساسية
Python 3.8+
مفتاح Gemini API Key (مطلوب فقط لمرحلة الإثراء).
خطوات التثبيت
تثبيت المكتبات:

pip install -r requirements.txt
إعداد مفتاح API: يجب تعيين مفتاح Gemini API كمتغير بيئة.
